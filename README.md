...
Follow these steps to set up an Android Emulator, launch it, and test the application.

### Step 1: Prerequisites in Android Studio
1. Open **Android Studio**.
2. Open **Tools** > **SDK Manager** (or click the settings gear / three dots on the welcome screen > **SDK Manager**).
3. In the **SDK Platforms** tab, ensure at least one modern Android SDK is installed (e.g., **Android 14.0 (UpsideDownCake)** or **Android 15.0 / 16.0**).
4. In the **SDK Tools** tab, make sure the following are checked and installed:
   - **Android SDK Build-Tools**
   - **Android Emulator**
   - **Android SDK Platform-Tools**
   - **Intel x86 Emulator Accelerator (HAXM)** (Intel CPUs) or **Android Emulator Hypervisor Driver** (AMD / Windows Hyper-V).
5. Click **Apply** / **OK** to complete installation.

### Step 2: Create an Android Virtual Device (AVD)
1. In Android Studio, go to **Tools** > **Device Manager** (or **Virtual Device Manager**).
2. Click **Create Device** (or the **+** button).
3. **Select Hardware**: Choose a phone profile (such as **Pixel 7**, **Pixel 8**, or **Medium Phone**) and click **Next**.
4. **Select System Image**:
   - Under the **Recommended** tab, click **Download** next to an API level (e.g., **API 34 / UpsideDownCake** with Google APIs or Google Play).
   - Once downloaded, select it and click **Next**.
5. **Verify Configuration**:
   - Give the device a name (e.g., `Medium_Phone`).
   - Keep default settings and click **Finish**.

### Step 3: Launch the Emulator
You can launch the emulator using either terminal commands or Android Studio:

#### Option A: From the Terminal (Recommended)
1. List all available virtual devices:
   ```powershell
   flutter emulators
   ```
2. Launch your emulator by ID / name:
   ```powershell
   flutter emulators --launch Medium_Phone
   ```
   *(Wait ~1-2 minutes until the virtual device finishes booting to the Android home screen).*

#### Option B: From Android Studio
- In the **Device Manager** panel, click the green **Play (▶)** button next to your virtual device.
...
1. Run the app:
   ...
   flutter run
   ```
   *(If multiple devices/browsers are active, specify the device ID: `flutter run -d emulator-5554`)*

2. Flutter will build the Android debug APK, install it on the emulator, and start the app.

#### Hot Reload & In-Session Shortcuts:
* Press **`r`** to **Hot Reload** (applies code changes instantly without restarting or losing state).
* Press **`R`** to **Hot Restart** (re-initializes app state).
* Press **`h`** to display all Flutter CLI shortcuts.
* Press **`q`** to quit and detach from the session.

---

## Testing Checklist on Android Emulator

Once the app opens on the emulator, verify the following core features:

1. **Authentication**:
   - Tap **Sign Up** to create a test account (enter email and 6+ character password).
   - Verify smooth transition to `HomeScreen` displaying your email badge in the AppBar.
2. **Initial Clean State**:
   - Confirm that no mock expenses are shown. You should see: *"No expenses logged yet. Tap + to add your first expense."*
3. **Add Expenses**:
   - Tap the **+** floating action button.
   - Enter an expense title, amount, and category (e.g. `Food`, `Transport`, `Bills`).
   - Tap **Save Expense** and verify the item appears in the list and the total spending card updates.
4. **Offline / Restart Persistence**:
   - Press **`R`** in the terminal to hot restart the app, or close and reopen the app from the emulator's app drawer.
   - Verify that all previously logged expenses and the running total are preserved.
5. **Delete Expense**:
   - Tap the trash icon next to an expense.
   - Verify that the item is deleted and the total spending card updates accordingly.
6. **Logout & User Isolation**:
   - Tap the logout button in the top right and confirm logout.
   - You will be redirected to the login screen.
   - Log in with a different account to confirm that each user's expense list is completely separate.
